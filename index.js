...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Hai ngab @${num.split('@')[0]} 👋🏻\nWelcome to group *${mdata.subject}*\n*Intro dulu ngab*\n*Nama :*\n*Umur :*\n*Askot :*\n*Gender :*\n*Yang betah ya ngab di sini sapa tau dapet doi:v*`
				...
				teks = `Sayonaraa beban👋🏻 @${num.split('@')[0]} jasamu akan di kubur dalam²:v`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
          ...
          const checkLimit = (sender) => {
           ...
           return false
            ...
            break
                ...
                addKoinUser('6281977578791@s.whatsapp.net', fee)
               ...
               client.updatePresence(from, Presence.composing) 
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				    try {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Jangan sampe sange ye ajg:v'})
					} catch (e) {
               ...
               case 'bucin':
				...
				case 'bcgc':
                   ...
                   if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					if (!isRegistered) return reply(ind.noregis())
					client.updatePresence(from, Presence.composing) 
					teks = `\`\`\`Ini adalah list group yXu BOT :\n\n\`\`\``
					no = 0
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					const bisa =['Bisa','Tidak Bisa','Coba Ulangi','Gatau anying','Mana gua tau asu gua bukan tuhan yg tau segalanya']
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					kapankah = body.slice(1)
					const kapan =['Besok','Lusa','Tadi','4 Hari Lagi','5 Hari Lagi','6 Hari Lagi','1 Minggu Lagi','2 Minggu Lagi','3 Minggu Lagi','1 Bulan Lagi','2 Bulan Lagi','3 Bulan Lagi','4 Bulan Lagi','5 Bulan Lagi','6 Bulan Lagi','1 Tahun Lagi','2 Tahun Lagi','3 Tahun Lagi','4 Tahun Lagi','5 Tahun Lagi','6 Tahun Lagi','1 Abad lagi','3 Hari Lagi','Kepo lu asu','Mana gua tau']
           ...
           if (isLimit(sender)) return reply(ind.limitend(pusname))
					apakah = body.slice(1)
					const apa =['Iya','Tidak','Bisa Jadi','Coba Ulangi','Gatau','Gatau anying','Pikir sendiri sono asu','Dahlah gatau','Iyaa ajg bener baget']
          ...
          if (!isRegistered) return reply(ind.noregis())
            ...
            const latensi = speed() - timestamp
               ...
               client.sendMessage(from, resul, text, { quoted: mek})
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
                reply(mess.wait)
                anu = await fetchJson(`https://api.vhtear.com/ytmp3?query=${body.slice(6)}&apikey=Beli Apikey ya sob! biar work`, {method: 'get'})
               if (anu.error) return reply(anu.error)
			...
			if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				case 'gif':
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply(`Kirim gambar/video dengan caption ${prefix}sticker/gif atau reply/tag gambar atau video`)
					}
					break
				case 'tts':
				if (!isRegistered) return reply(ind.noregis())
				...
				case 'clearall':
				 ...
				 client.chatRead (from)
					...
					client.sendMessage(from, `𝗽𝗲𝗿𝗶n𝘁𝗮𝗵 𝗗𝗶𝘁𝗲𝗿𝗶𝗺𝗮, 𝗺𝗲𝗺𝗯𝘂𝗸𝗮 ${body.slice(9)}@c.us`, text)
					break
				case 'leave': 
				if (!isGroup) return reply(ind.groupo())
					if (!isOwner) return reply(ind.ownerb())
				...
				case 'bc': 
			   	...
			   	case 'setpp': 
                       ...
                       media = await client.downloadAndSaveMediaMessage(mek)
                        ...
                        reply('s𝘂𝗸𝘀𝗲𝘀 𝗺𝗲𝗻𝗴𝗴𝗮𝗻𝘁𝗶 𝗶𝗰𝗼𝗻 𝗚𝗿𝘂𝗽')
					break						
				case 'add':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					...
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (args[0] === 'buka') {
					    reply(`𝐒𝐔𝐂𝐂𝐄𝐒 𝐎𝐏𝐄𝐍 𝐆𝐑𝐎𝐔𝐏`)
						client.groupSettingChange(from, GroupSettingChange.messageSend, false)
					} else if (args[0] === 'tutup') {
						reply(`𝐒𝐔𝐂𝐂𝐄𝐒 𝐂𝐋𝐎𝐒𝐄𝐃 𝐆𝐑𝐎𝐔𝐏`)
            ...
            case 'creator':
                  ...
                  client.sendMessage(from, 'Langsung save aja ngab owner nyaa cogan:v',MessageType.text, { quoted: mek} )
					break    
           case 'setname':
                if (!isGroup) return reply(ind.groupo())
			    if (!isGroupAdmins) return reply(ind.admin())
           ...
           case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`𝘆𝗮𝗵𝗵 @${mentioned[0].split('@')[0]} 𝗷𝗮batan 𝗮𝗱𝗺𝗶𝗻 𝗸𝗮𝗺𝘂 𝘀u𝗱𝗮𝗵 𝗱𝗶 𝗰𝗼𝗽𝗼𝘁🏃`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'promote':
				...
				if (!isQuotedSticker) return reply('𝗥𝗲𝗽𝗹𝘆/𝘁𝗮𝗴 𝘀𝘁𝗶𝗰𝗸𝗲𝗿 !')
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						if (err) return reply(ind.stikga())
						buffer = fs.readFileSync(ran)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nohh, bilang apaa ama aing:v'})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender)
					break
			...
			if (body.startsWith(`${prefix}${command}`)) {
                  ...
                  reply(`*「 EROR 」*\n\nMaaf *${pushname}*\n*${prefix}${command}* Command is not on the *${prefix}menu*!`)
